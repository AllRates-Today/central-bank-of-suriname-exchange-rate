# Central Bank of Suriname Exchange Rates API — central-bank-of-suriname-exchange-rate

[![npm version](https://img.shields.io/npm/v/central-bank-of-suriname-exchange-rate.svg)](https://www.npmjs.com/package/central-bank-of-suriname-exchange-rate)
...
> **Official rate, not mid-market:** every value here is a number Central Bank of Suriname itself published, fixed once printed and carrying the central bank's own `rate_date` — what filings and audits require. Need the live interbank midpoint for pricing or display instead? Use the [mid-market API](https://allratestoday.com/docs/) or [`@allratestoday/sdk`](https://www.npmjs.com/package/@allratestoday/sdk). The two can diverge by several percent.

## ⚡ Try it without a key

The latest Central Bank of Suriname table is also served keyless, CORS-open and edge-cached, for evaluation, embeds and AI agents:

```bash
curl "https://allratestoday.com/api/open/central-bank/cbvs?source=USD&target=SRD"
```

```js
const r = await fetch('https://allratestoday.com/api/open/central-bank/cbvs').then((x) => x.json());
...
```bash
yarn add central-bank-of-suriname-exchange-rate
```

```bash
pnpm add central-bank-of-suriname-exchange-rate
```

Requires Node 18+ (global `fetch`); also runs on Bun, Deno and edge runtimes. Also published under the org scope as [`@allratestoday/central-bank-of-suriname-exchange-rate`](https://www.npmjs.com/package/@allratestoday/central-bank-of-suriname-exchange-rate) — same code, same versions.
...
console.log(pair.rate, pair.rate_date); // the official Central Bank of Suriname rate, on the central bank's own date
```
...
```js
import { getLatestRates } from 'central-bank-of-suriname-exchange-rate';
...
const one = await getRatesForDate('2026-06-30', { apiKey: 'art_live_...', source: 'USD', target: 'SRD' });
```
...
Central Bank of Suriname currently publishes rates covering **11 currencies** against the SRD (as of the latest table):

🇦🇼 `AWG` · 🇧🇧 `BBD` · 🇧🇷 `BRL` · 🇨🇳 `CNY` · 🇪🇺 `EUR` · 🇬🇧 `GBP` · 🇬🇾 `GYD` · 🇹🇹 `TTD` · 🇺🇸 `USD` · `XCD` · `XCG`
...
- Publisher's own page: [Weighted-average exchange rates](https://www.cbvs.sr/) · [www.cbvs.sr](https://www.cbvs.sr)
- Publication: every business day; the exact schedule, freshness status and any current delay are on the [Central Bank of Suriname rates page](https://allratestoday.com/central-bank-rates-api/cbvs/)
...
- Central Bank of Suriname quotes **SRD per 1 unit of foreign currency** (e.g. `base: "USD", quote: "SRD"` means SRD per one US dollar).
...
Loading the official Central Bank of Suriname rate into an accounting system is a supported workflow, not a hack. Step-by-step guides with the direction each system expects:
...
```bash
curl "https://allratestoday.com/api/v1/central-bank/cbvs/latest?format=xml&api_key=art_live_..."
```
...
If Central Bank of Suriname does not print a pair directly, the API resolves it from the central bank's own table and says so — official and computed values are never confused:

| `method` | `derived` | Meaning |
| --- | --- | --- |
| `published` | `false` | The central bank printed this pair directly |
| `inverse` | `true` | Computed as 1 ÷ the published opposite direction |
| `cross` | `true` | Computed via SRD from two published rates |
...
| `404` | Pair or date range not covered by Central Bank of Suriname |
...
```ts
import type { LatestRates, PairRate, DatedRates, RateEntry, HistoryQuery, RequestOptions } from 'central-bank-of-suriname-exchange-rate';
```

## 📦 CommonJS

```javascript
const { getRate } = require('central-bank-of-suriname-exchange-rate');

getRate('USD', 'SRD', { apiKey: 'art_live_...' }).then((pair) => console.log(pair.rate));
```
...
- [Central Bank of Suriname rates page](https://allratestoday.com/central-bank-rates-api/cbvs/) — live table, publication cadence, FAQ
- [All central bank sources](https://allratestoday.com/central-bank-rates-api/)
- [Package docs on the site](https://allratestoday.com/docs/sdk/central-bank-of-suriname-exchange-rate/) · [ERP integration guides](https://allratestoday.com/docs/integrations/)
...
- [GitHub](https://github.com/AllRates-Today/central-bank-of-suriname-exchange-rate)
